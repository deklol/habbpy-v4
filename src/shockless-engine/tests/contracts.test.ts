...
test("plugin worker APIs exposed to user plugins have renderer request handlers", () => {
  ...
  const appSource = readFileSync(join(process.cwd(), "src", "renderer", "ui", "App.tsx"), "utf8");
  const exposed = [...hostSource.matchAll(/request\("([^"]+)"/g)].map((match) => match[1]!).sort();
  const uniqueExposed = [...new Set(exposed)];
  const missing = uniqueExposed.filter((api) => !appSource.includes(`request.api === "${api}"`));
  assert.deepEqual(missing, []);
});

test("notification plugin API uses the documented notification permission", () => {
  const appSource = readFileSync(join(process.cwd(), "src", "renderer", "ui", "App.tsx"), "utf8");
  const block = appSource.match(/if \(request\.api === "notifications\.showBulletin"\) \{[\s\S]*?throw new Error\(`Unknown plugin host API/);
...
test("plugin icon documentation matches renderer icon map", () => {
  const helperSource = readFileSync(join(process.cwd(), "src", "renderer", "ui", "helpers.tsx"), "utf8");
  const iconMapBlock = helperSource.match(/export const iconMap = \{([\s\S]*?)\};/);
  assert.ok(iconMapBlock, "iconMap must be exported from renderer helpers");
  ...
  const authoring = readFileSync(join(process.cwd(), "docs", "plugin-authoring.md"), "utf8");
  const apiReference = readFileSync(join(process.cwd(), "docs", "plugin-api-reference.md"), "utf8");
  const wiki = readFileSync(join(process.cwd(), "docs", "plugin-api.html"), "utf8");
  for (const key of iconKeys) {
    assert.match(authoring, new RegExp(`- \`${key}\``), `plugin-authoring.md missing icon ${key}`);
    assert.match(apiReference, new RegExp(`- \`${key}\``), `plugin-api-reference.md missing icon ${key}`);
...
test("plugin API documentation covers every exposed worker request API", () => {
  ...
  const missingFromReference = exposedApis.filter((api) => !apiReference.includes(api));
