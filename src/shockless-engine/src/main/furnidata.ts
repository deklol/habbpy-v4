...
export async function readFurniMetadataSnapshot(appDataPath: string): Promise<FurniMetadataSnapshot> {
  const cachePath = firstExistingAppDataStorePath(appDataPath, "gamedata", FURNIDATA_CACHE_FILE);
  const writeCachePath = appDataStorePath(appDataPath, "gamedata", FURNIDATA_CACHE_FILE);
  let text = "";
