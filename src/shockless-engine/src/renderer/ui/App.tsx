...
import { pluginSchemaActionGate } from "./pluginSurfaceGuards";
...
const pluginUiValuesStorageKey = "habbpy-v4:plugin-ui-values";

type SchemaPrimitiveValue = string | number | boolean | null;

function loadPersistedPluginRuntimeUiValues(): Readonly<Record<string, RuntimePluginUiState | undefined>> {
  if (typeof window === "undefined") return {};
  try {
    const parsed = JSON.parse(window.localStorage.getItem(pluginUiValuesStorageKey) || "{}") as unknown;
...
function pluginRuntimeUiValueSnapshot(
  state: Readonly<Record<string, RuntimePluginUiState | undefined>>,
...
export function App() {
  ...
  userPluginRequestHandlerRef.current = async (plugin, request) => {
    ...
    if (request.api === "plants.runCycle") {
      ...
      requirePluginPermission(plugin, ["engine.snapshot"]);
      if (!window.habbpyV4) throw new Error("Desktop bridge unavailable for plant cycle.");
      const options = args.options && typeof args.options === "object" ? (args.options as Record<string, unknown>) : {};
      const targetClientId = requestedPluginClientId(args, selectedClientIdRef.current);
      const snapshot = await fullSnapshotForClient(targetClientId);
      const plan = pluginPlantCyclePlan(snapshot, args.selector ?? options.selector ?? {}, furniMetadata);
      const planRecord = plan as Record<string, unknown> | null;
      const objectId = cleanPositiveInt(planRecord?.["objectId"], 0);
      const originalValue = planRecord?.["original"];
      const workingValue = planRecord?.["working"];
      const original = originalValue && typeof originalValue === "object" ? originalValue as Record<string, unknown> : {};
      const working = workingValue && typeof workingValue === "object" ? workingValue as Record<string, unknown> : {};
      if (!objectId) throw new Error("plants.runCycle could not resolve a plant object id.");
      const originalX = cleanInteger(original.x, 0);
      const originalY = cleanInteger(original.y, 0);
      const originalDirection = cleanInteger(original.direction, 0);
      const workingX = cleanInteger(working.x, originalX);
      const workingY = cleanInteger(working.y, originalY);
      const workingDirection = cleanInteger(working.direction, originalDirection);
      const mode = String(options.mode ?? args.mode ?? "waterHarvest").trim().toLowerCase();
      const settleMs = Math.max(0, Math.min(5000, cleanInteger(options.settleMs ?? args.settleMs, 700)));
      const actionDelayMs = Math.max(0, Math.min(5000, cleanInteger(options.actionDelayMs ?? args.actionDelayMs, 900)));
      const results: unknown[] = [];
      results.push(await window.habbpyV4.sendGardeningRelayAction({ action: "move", objectId, x: workingX, y: workingY, direction: workingDirection }, targetClientId));
      if (settleMs > 0) await delay(settleMs);
      if (mode === "compost" || mode === "compostonly" || mode === "compost-only") {
        results.push(await window.habbpyV4.sendGardeningRelayAction({ action: "compost", objectId }, targetClientId));
      } else {
        results.push(await window.habbpyV4.sendGardeningRelayAction({ action: "water", objectId }, targetClientId));
        if (actionDelayMs > 0) await delay(actionDelayMs);
        results.push(await window.habbpyV4.sendGardeningRelayAction({ action: "harvest", objectId }, targetClientId));
      }
      if (actionDelayMs > 0) await delay(actionDelayMs);
      results.push(await window.habbpyV4.sendGardeningRelayAction({ action: "move", objectId, x: originalX, y: originalY, direction: originalDirection }, targetClientId));
      await Promise.all([refreshRuntimeSnapshot().catch(() => null), refreshRelayLog().catch(() => null)]);
      return { ok: true, message: `Plant cycle sent for #${objectId}.`, plan, results };
    }
  ...
  const executePacketConsoleCommand = useCallback(async () => {
    ...
    if (command === "help" || command === "?") {
      appendPacketConsole(
        "info",
        "commands: help, clear, packets <filter|all|selected|client id>, list, select <id>, newclient, load <file> <count> --headless, mimic status|on|off|source <id>, enterroom <flat-id>, room, user, lookup <name>, rooms <query>, showNames true|false, say <message>, input [client] <message>, wave, dance <1-4>, carrydrink, message <user|id> <message>, adduser <name>, requests, accept <request>, decline <request>, follow <friend>, removefriend <friend>, walk <x> <y>, fps [limit], perf, gpu",
      );
      return;
    }
    if (command === "clear") {
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(pluginUiValuesStorageKey, JSON.stringify(pluginRuntimeUiValueSnapshot(pluginRuntimeUiById)));
    } catch {
  ...
  useEffect(() => {
    const webview = webviewRef.current;
    if (!webview || !engineUrl || !selectedClientIsVisible) return;
    if (!engineUserNameLabels) {
      void runEngineRuntimeAction(webview, { kind: "setUserNameLabels", enabled: false, settings: userNameLabelSettings });
      return;
    }
    let cancelled = false;
    let inFlight = false;
    ...
    const applyNameLabels = async () => {
      ...
      inFlight = true;
      try {
        const result = await runEngineRuntimeAction(webview, { kind: "setUserNameLabels", enabled: true, settings: userNameLabelSettings });
  ...
  }, [engineUrl, engineUserNameLabels, selectedClientId, selectedClientIsVisible, userNameLabelSettings]);
  ...
  return (
    <div className="about-overlay" role="presentation" onMouseDown={onClose}>
      <section className="about-modal" role="dialog" aria-modal="true" aria-label="About Shockless" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="about-credits">Shockless Engine / ProjectorRays</div>
