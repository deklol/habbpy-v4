  ...
  private correctWrapperSpriteLocations(
    ...
    for (let index = 0; index < wrappedParts.values.length; index += 1) {
      ...
      if (!baseline || baseline.instance !== wrapper) {
        baseline = this.captureWrapperBaseline(wrapper, offsetX, offsetY, appliedX, appliedY);
        roomStage.wrappers.set(baselineKey, baseline);
        if (baseline.parts.length > 0) {
          anchors.push({
            id: `wrapper:${String(wrappedParts.keys[index] ?? index + 1)}`,
            kind: "sprite",
            action: "wrapper-late-source-baseline",
            x: sprite.locH,
            y: sprite.locV,
            note: typeDef,
          });
      ...
      if (contentShift && this.matchesRoomStageMove(contentShift, wrapperAppliedX, wrapperAppliedY)) {
        const expectedSourceRenderedX = Math.round(offsetX);
        const expectedSourceRenderedY = Math.round(offsetY);
        if (
          this.spriteAt(sprite, expectedSpriteMovedX, expectedSpriteMovedY) ||
          this.spriteAt(sprite, expectedSourceRenderedX, expectedSourceRenderedY)
        ) {
          continue;
        }
        anchors.push({
