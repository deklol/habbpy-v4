...
function collectViableBitmapSources(candidates, logical, element) {
  ...
  for (const member of candidates) {
    ...
    const palette = bitmap.bitDepth <= 8 ? resolvePalette(member.castName, element.palette, bitmap.paletteId) : undefined;
    viable.push({
      member,
      bitmap,
      palette,
...
function bitmapAssetVariantSuffix(element) {
  ...
  if (!normalized || normalized === "interface palette" || normalized === "systemmac" || normalized === "systemwin" || normalized === "systemwindir4" || normalized === "grayscale") {
    return "";
  }
