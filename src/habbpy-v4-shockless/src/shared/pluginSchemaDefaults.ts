...
function defaultSurfaceLayout(plugin: PluginDefinition, surface: PluginUiSurface): readonly PluginUiElement[] {
  ...
  if (surface.kind === "commands") {
    return [
      ...
      commandTable(plugin),
      hotkeyTable(plugin),
      plugin.commands?.length ? { type: "notice", tone: "info", text: "Commands are routed through the Habbpy command registry or backtick console." } : { type: "text", tone: "default", text: "No commands declared for this plugin." },
    ];
  }
